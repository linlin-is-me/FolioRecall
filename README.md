...
当前为 **0.1.0rc2 本地预发布候选**。第四阶段核心验收完成，GitHub代码、标签及Release尚未公开上传。下文固定版本下载命令供发布后使用；当前本机候选位于 `outputs/stage4/release-v0.1.0rc2`，可将其中同名资产复制到工作目录执行安装。发布及后续状态以[开发计划第1节](doc/多模态文档检索项目开发计划.md#当前开发重点)为准。

## 实际使用

![CPU安装态真实检索演示](doc/assets/stage4-cpu-demo.png)

42页欧盟报告示例的查询命中人口报告物理第10页Figure 3，页面预览和下载JSON与CLI一致。范围外问题仍可能返回无关页面，目前不提供无答案检测、答案生成或中文能力声明。
...
访问 `http://127.0.0.1:7860`。服务仅监听本机，持续加载一个学生和一个索引，切换配置后重启。单次CLI会重新加载模型。历史CPU实测首次加载约44秒，暖态页面显示准备P50/P95为272/285ms，Gradio客户端响应约1.038/1.069秒；这些边界不包含浏览器页面传输和渲染，不能用核心编码延迟替代界面耗时。
